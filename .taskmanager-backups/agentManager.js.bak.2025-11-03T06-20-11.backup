...
class AgentManager {
  ...
  async unregisterAgent(_agentId) {
    ...
    if (agent.assignedTasks && agent.assignedTasks.length > 0) {
      await this.releaseAllTasksFromAgent(_agentId);
    }
  ...
  /**
     ...
     * @returns {boolean} Success status
     */
  async updateAgentHeartbeat(_agentId) {
    try {
      const todoData = await this.readTodo();
  ...
  /**
     ...
     */
  async canAgentAcceptTasks(_agentId) {
    const agent = await this.getAgent(_agentId);
  ...
  /**
     ...
     */
  async releaseAllTasksFromAgent(_agentId) {
    const todoData = await this.readTodo();
    const releasedTasks = [];
  ...
  /**
     ...
     * @param {string} agentId - Agent ID
     */
  startHeartbeatMonitoring(_agentId) {
    this.agentHeartbeats.set(agentId, Date.now());
  ...
  /**
     ...
     * @param {string} agentId - Agent ID
     */
  stopHeartbeatMonitoring(_agentId) {
    this.agentHeartbeats.delete(_agentId);
  ...
  async markAgentsAsInactive(agentIds) {
    ...
    const inactivationPromises = agentIds.map(async (_agentId) => {
      // Security: Validate agentId to prevent object injection
      if (!this.isValidAgentId(_agentId)) {
  ...
  isValidAgentId(_agentId) {
    ...
    if (!agentIdPattern.test(_agentId)) {
      return false;
    }
