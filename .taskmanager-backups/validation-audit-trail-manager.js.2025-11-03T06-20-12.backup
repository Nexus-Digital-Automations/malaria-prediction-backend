...
class ValidationAuditTrailManager {
  ...
  startAuthorizationSession(agentId, authorizationKey, requiredSteps = []) {
    try {
      ...
      loggers.stopHook.info(`🔍 Started audit session ${sessionId} for agent ${agentId}`);
  ...
  completeAuthorizationSession(sessionId, finalStatus = 'completed') {
    try {
      ...
      loggers.stopHook.info(`✅ Authorization session ${sessionId} completed with status: ${finalStatus}`);
