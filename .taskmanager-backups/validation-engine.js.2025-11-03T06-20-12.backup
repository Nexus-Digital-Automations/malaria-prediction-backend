...
class ValidationEngine {
  ...
  validateSecurityReview(_VALIDATION_CONTEXT) {
    try {
      ...
      try {
        ...
        // Failed to parse audit output - error intentionally ignored
      }
      ...
      const fileChecks = await Promise.allSettled(
        configFiles.map(async (file) => {
          try {
            ...
            return file;
            // eslint-disable-next-line no-unused-vars -- Catch parameter intentionally unused for Promise.allSettled pattern
          } catch (_error) {
            return null;
          }
        }),
  ...
  countFilesByExtension(extensions) {
    try {
      ...
      // eslint-disable-next-line no-unused-vars -- Catch parameter intentionally unused for fallback return
    } catch (_error) {
      return 0;
    }
  }
