...
class EnhancedPerformanceCollector {
  constructor(projectRoot, _agentId) {
    ...
    this.resourceMonitor.onAlert((_alerts) => {
      loggers.stopHook.warn('Resource usage alerts:', _alerts);
    });
   ...
   */
  async startCollection(criterion, options = {}, _agentId) {
    try {
      ...
      const _systemInfo = await this._getSystemInfo();
      ...
      const resourceMonitorResult = await this.resourceMonitor.startMonitoring();
   ...
   */
  recordFileAccess(filename, accessType = 'read') {
    if (!this.currentMetrics) {
      return;
    }

    const normalizedPath = path.relative(this.projectRoot, filename);
  ...
  _integrateResourceMonitorData(resourceSummary) {
    if (!this.currentMetrics || !resourceSummary.success) {
      return;
    }
  ...
  _calculatePerformanceGrade(duration) {
    ...
    if (duration <= this.performanceThresholds.acceptable) {
      return 'C';
    }
    if (duration <= this.performanceThresholds.poor) {
      return 'D';
    }
    return 'F';
  }
