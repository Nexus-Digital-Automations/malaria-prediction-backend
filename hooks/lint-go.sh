...
run_go_direct_lint() {
    ...
    if command_exists golangci-lint; then
        ...
        # First attempt autofix with golangci-lint
        log_debug "Attempting golangci-lint autofix..."
        local fix_output
        local fix_cmd="golangci-lint run --timeout=2m${exclude_args} --fix"
        fix_output=$($fix_cmd 2>&1 || true)
        log_to_file "golangci-lint autofix output: $fix_output"
        ...
        if ! lint_output=$($lint_cmd 2>&1); then
            add_error "golangci-lint found issues (after autofix attempt)"
            echo "$lint_output" >&2
        fi
