...
# Ensure minimal setup is present (non-disruptive)
if [[ -f "${SCRIPT_DIR}/ensure-setup.sh" ]]; then
    bash "${SCRIPT_DIR}/ensure-setup.sh" >/dev/null 2>&1 || true
fi
...
add_error() {
    ...
    # Store errors for lint results file instead of immediate printing
}

print_summary() {
    if [[ $CLAUDE_HOOKS_ERROR_COUNT -gt 0 ]]; then
        # Generate fresh lint results for detailed viewing
        "${SCRIPT_DIR}/lint-results-generator.sh" >/dev/null 2>&1 || true
        
        # Concise summary with reference to detailed file - output to stderr for Claude Code visibility
        echo "" >&2
        echo -e "⚠️  **Code Quality Alert:** ❌ $CLAUDE_HOOKS_ERROR_COUNT errors found" >&2
        echo "" >&2
        echo -e "🚨 **READ IMMEDIATELY:** \`development/reports/lint-results.md\`" >&2
        echo "" >&2
    fi
}
    ...
    if command_exists black; then
        ...
        if ! echo "$filtered_files" | xargs black --check >/dev/null 2>&1; then
            ...
            if ! format_output=$(echo "$filtered_files" | xargs black 2>&1); then
                ...
                # Details saved to lint results file
            fi
        fi
    fi
...
lint_javascript() {
    ...
    # Check for ESLint with autofix
    if [[ -f "package.json" ]] && grep -q "eslint" package.json 2>/dev/null; then
        if command_exists npm; then
            # First try to auto-fix with npm script
            if grep -q "lint:fix" package.json 2>/dev/null; then
                log_debug "Attempting ESLint autofix with npm run lint:fix..."
                local fix_output
                fix_output=$(npm run lint:fix --if-present 2>&1 || true)
                log_to_file "ESLint autofix output: $fix_output"
            fi
            ...
            if [[ $eslint_exit_code -ne 0 ]]; then
                add_error "ESLint found issues (after autofix attempt)"
                echo "$eslint_output" >&2
            fi
        elif command_exists npx; then
            # Fallback to direct ESLint with autofix
            log_debug "Attempting ESLint autofix with npx..."
            local fix_output
            fix_output=$(echo "$filtered_files" | xargs npx eslint --fix 2>&1 || true)
            log_to_file "ESLint direct autofix output: $fix_output"
            
            # Then check for remaining issues
            local eslint_output
            if ! eslint_output=$(echo "$filtered_files" | xargs npx eslint 2>&1); then
                add_error "ESLint found issues (after autofix attempt)"
                echo "$eslint_output" >&2
            fi
        fi
    ...
    local ts_files
    ts_files=$(echo "$filtered_files" | tr ' ' '\n' | grep -E '\.(ts|tsx)$' | tr '\n' ' ')
    
    if [[ -n "$ts_files" ]] && [[ -f "tsconfig.json" ]]; then
        if command_exists tsc; then
            log_debug "Running TypeScript type checking..."
            local tsc_output
            ...
            log_debug "Running TypeScript type checking via npx..."
            local tsc_output
    ...
    if command_exists shfmt; then
        ...
        for file in $shell_files; do
            ...
            if ! shfmt -d "$file" >/dev/null 2>&1; then
                log_debug "Attempting shfmt autofix for: $file"
                # Attempt to auto-format the file
                if shfmt -w -i 2 -ci -sr "$file" 2>/dev/null; then
                    log_debug "Successfully auto-formatted: $file"
                    # Re-check after formatting
                    if ! shfmt -d "$file" >/dev/null 2>&1; then
                        format_errors=true
                        echo -e "${RED}❌ Formatting issues remain in: $file${NC}" >&2
                        add_error "Shell formatting issues in $file (after autofix attempt)"
                    fi
                else
                    format_errors=true
...
# Auto-setup function to run the setup script when hook fails
run_auto_setup() {
    ...
    if [[ "$project_root" == "$claude_projects_root"/* ]]; then
        log_debug "Project is within Claude Coding Projects directory, attempting auto-setup..."
        
        # Get the project folder name
        local project_name
        project_name=$(basename "$project_root")
        local project_path="$claude_projects_root/$project_name"
        
        log_debug "Running setup script for project: $project_path"
        
        # Look for setup scripts in the post-tool-stop-hook project
        local setup_script="$claude_projects_root/post-tool-stop-hook/setup-post-tool-feedback-all.sh"
        
        if [[ -f "$setup_script" ]]; then
            log_debug "Found setup script: $setup_script"
            echo -e "${YELLOW}🔧 Hook setup incomplete. Running auto-setup...${NC}" >&2
            
            # Run the setup script (suppress most output, only show errors)
            if bash "$setup_script" >/dev/null 2>&1; then
                echo -e "${GREEN}✅ Auto-setup completed successfully${NC}" >&2
                echo -e "${BLUE}💡 Please retry your last action${NC}" >&2
                return 0
            else
                echo -e "${RED}❌ Auto-setup failed${NC}" >&2
                echo -e "${YELLOW}💡 Manual setup may be required${NC}" >&2
                return 1
            fi
        else
            log_debug "Setup script not found at: $setup_script"
            return 1
        fi
    else
        log_debug "Project is not within Claude Coding Projects directory: $project_root"
        return 1
    fi
}
...
if [[ $exit_code -eq 2 ]]; then
    # Check if this is a setup-related failure and try auto-setup
    if [[ ! -f "${PROJECT_ROOT}/development/reports/lint-results.md" ]] || 
       [[ ! -f "${SCRIPT_DIR}/common-helpers.sh" ]] ||
       [[ ! -f "${SCRIPT_DIR}/lint-results-generator.sh" ]]; then
        
        log_debug "Detected missing setup files, attempting auto-setup"
        if run_auto_setup "$PROJECT_ROOT"; then
            # Auto-setup succeeded, exit with special code to indicate retry needed
            exit 3
        fi
    fi
    
    # No additional message needed - summary already provided
    exit 2
else
