...
run_safe_lint_checks() {
    ...
    if [[ -n "$(find . -name "*.sh" -type f | head -10)" ]]; then
        echo "#### Shell Scripts"
        local shell_files=$(find . -name "*.sh" -type f | grep -v -E "(\.git/|node_modules/)" | head -10)
        
        for file in $shell_files; do
            # Skip files larger than 512KB to prevent buffer overflow
            if [[ -f "$file" ]] && [[ $(stat -f%z "$file" 2>/dev/null || echo 0) -gt 524288 ]]; then
                echo "⚠️ **Skipped (>512KB):** $file"
                warnings=$((warnings + 1))
                continue
            fi
            
            # Check for basic syntax issues with timeout
            if timeout 5s bash -n "$file" 2>/dev/null; then
                echo "✅ **Syntax OK:** $file"
            else
    ...
    if [[ -n "$(find . -name "*.py" -type f | head -5)" ]]; then
        echo "#### Python Files"
        local py_files=$(find . -name "*.py" -type f | grep -v -E "(\.git/|__pycache__/|venv/)" | head -5)
        
        for file in $py_files; do
            # Skip files larger than 1MB to prevent buffer overflow
            if [[ -f "$file" ]] && [[ $(stat -f%z "$file" 2>/dev/null || echo 0) -gt 1048576 ]]; then
                echo "⚠️ **Skipped (>1MB):** $file"
                warnings=$((warnings + 1))
                continue
            fi
            
            # Check for basic syntax with timeout
            if timeout 5s python3 -m py_compile "$file" 2>/dev/null; then
                echo "✅ **Syntax OK:** $file"
            else
                echo "❌ **Syntax Error:** $file"
                errors=$((errors + 1))
    ...
    if [[ -n "$(find . -name "*.js" -type f | head -5)" ]]; then
        ...
        local js_files=$(find . -name "*.js" -type f | grep -v -E "(\.git/|node_modules/)" | head -5)
        ...
        for file in $js_files; do
            ...
            if command -v node >/dev/null 2>&1; then
                # Use timeout and limit output to prevent buffer overflow
                if timeout 5s node -c "$file" 2>/dev/null; then
                    echo "✅ **Syntax OK:** $file"
                else
                    echo "❌ **Syntax Error:** $file"
                    errors=$((errors + 1))
    ...
    if [[ -n "$(find . -name "*.go" -type f | head -5)" ]]; then
        echo "#### Go Files"
        local go_files=$(find . -name "*.go" -type f | head -5)
        ...
        if command -v go >/dev/null 2>&1; then
            for file in $go_files; do
                ...
                # Check format with timeout
                if timeout 5s go fmt -l "$file" >/dev/null 2>&1; then
                    echo "✅ **Format OK:** $file"
                else
