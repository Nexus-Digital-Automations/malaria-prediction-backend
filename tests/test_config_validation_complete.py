...
class TestConfigValidator:
    ...
    def mock_settings(self):
        ...
        settings.data_path = Path("/tmp/test_data")
        settings.model_path = Path("/tmp/test_models")
    ...
    def test_validate_all_success(self, validator):
        ...
        with (
            patch.object(validator, "_validate_configuration") as mock_validate_config,
    ...
    def test_validate_all_with_configuration_exception(self, validator):
        """Test validation with configuration validation exception."""
        with (
            patch.object(validator, "_validate_configuration") as mock_validate_config,
            patch.object(validator, "_validate_secrets") as mock_validate_secrets,
        ):
            mock_validate_config.side_effect = Exception("Config validation failed")
            mock_validate_secrets.return_value = {"status": "pass"}

            result = validator.validate_all(include_health_checks=False)

            assert "error" in result["configuration_validation"]
            assert result["validation_summary"]["failed"] >= 1

    def test_validate_all_with_secrets_exception(self, validator):
        """Test validation with secrets validation exception."""
        with (
            patch.object(validator, "_validate_configuration") as mock_validate_config,
            patch.object(validator, "_validate_secrets") as mock_validate_secrets,
        ):
            mock_validate_config.return_value = {"status": "pass"}
            mock_validate_secrets.side_effect = Exception("Secrets validation failed")

            result = validator.validate_all(include_health_checks=False)

            assert "error" in result["secrets_validation"]
            assert result["validation_summary"]["failed"] >= 1

    def test_validate_all_with_health_checks_exception(self, validator):
        """Test validation with health checks exception."""
        with (
            patch.object(validator, "_validate_configuration") as mock_validate_config,
            patch.object(validator, "_validate_secrets") as mock_validate_secrets,
            patch("asyncio.run") as mock_asyncio_run,
            ...
            mock_validate_config.return_value = {"status": "pass"}
            mock_validate_secrets.return_value = {"status": "pass"}
            mock_asyncio_run.side_effect = Exception("Health checks failed")

            result = validator.validate_all()

            assert "error" in result["health_checks"]
            assert result["validation_summary"]["failed"] >= 1

    def test_validate_configuration(self, validator):
        """Test configuration validation."""
        with (
            patch.object(validator, "_validate_environment") as mock_env,
            patch.object(validator, "_validate_paths") as mock_paths,
            patch.object(validator, "_validate_urls") as mock_urls,
            patch.object(validator, "_validate_constraints") as mock_constraints,
            patch.object(validator, "_validate_security") as mock_security,
        ):
            mock_env.return_value = {"status": "pass"}
            mock_paths.return_value = {"status": "pass"}
            mock_urls.return_value = {"status": "pass"}
            mock_constraints.return_value = {"status": "pass"}
            mock_security.return_value = {"status": "pass"}
    ...
    def test_validate_paths_success(self, validator):
        ...
        with (
            ...
            assert result["data_path"]["status"] == "pass"
            assert result["model_path"]["status"] == "pass"

    def test_validate_paths_missing_data_path(self, validator):
        """Test path validation with missing data path."""
        with (
            patch(
                "pathlib.Path.exists",
            ...
            assert result["data_path"]["status"] == "fail"

    def test_validate_paths_not_directory(self, validator):
        """Test path validation with path that's not a directory."""
        with (
            patch("pathlib.Path.exists", return_value=True),
            ...
            assert result["data_path"]["status"] == "fail"
            assert result["model_path"]["status"] == "fail"
    ...
    def test_validate_urls_invalid_database_url(self, validator):
        """Test URL validation with invalid database URL."""
        validator.settings.database_url = "invalid_url"

        result = validator._validate_urls()

        assert result["database_url"]["status"] == "fail"

    def test_validate_urls_invalid_redis_url(self, validator):
        """Test URL validation with invalid Redis URL."""
        validator.settings.redis_url = "invalid_url"

        result = validator._validate_urls()

        assert result["redis_url"]["status"] == "fail"
    ...
    def test_validate_constraints_success(self, validator):
        ...
        assert result["max_request_size"]["status"] == "pass"
        assert result["timeout"]["status"] == "pass"
        assert result["workers"]["status"] == "pass"

    def test_validate_constraints_invalid_request_size(self, validator):
        """Test constraints validation with invalid request size."""
        validator.settings.max_request_size = -1

        result = validator._validate_constraints()

        assert result["max_request_size"]["status"] == "fail"

    def test_validate_constraints_invalid_timeout(self, validator):
        """Test constraints validation with invalid timeout."""
        validator.settings.timeout = 0

        result = validator._validate_constraints()

        assert result["timeout"]["status"] == "fail"

    def test_validate_constraints_invalid_workers(self, validator):
        """Test constraints validation with invalid workers count."""
        validator.settings.workers = 0

        result = validator._validate_constraints()

        assert result["workers"]["status"] == "fail"
    ...
    def test_validate_security_success(self, validator):
        ...
        assert result["secret_key"]["status"] == "pass"

    def test_validate_security_weak_secret_key(self, validator):
        """Test security validation with weak secret key."""
        validator.settings.secret_key = "weak"

        result = validator._validate_security()

        assert result["secret_key"]["status"] == "fail"

    def test_validate_security_no_secret_key(self, validator):
        """Test security validation with no secret key."""
        validator.settings.secret_key = None

        result = validator._validate_security()

        assert result["secret_key"]["status"] == "fail"

    def test_validate_secrets_success(self, validator):
        """Test secrets validation success."""
    ...
    def test_validate_secrets_failure(self, validator):
    ...
    async def test_check_database_health_success(self, validator):
        ...
        with patch(
            ...
            # Also mock the get_database_url method
            validator.settings.get_database_url = Mock(
                return_value="postgresql://user:pass@localhost:5432/test_db"
            )
    ...
    async def test_check_external_apis_health_success(self, validator):
        ...
        with patch("httpx.AsyncClient") as mock_client_class:
            ...
            assert "era5_api" in result
            assert "modis_api" in result
            assert "chirps_api" in result
    ...
    @pytest.mark.asyncio
    async def test_check_external_apis_health_failure(self, validator):
        """Test external APIs health check with failures."""
        with patch("httpx.AsyncClient") as mock_client_class:
            mock_client = AsyncMock()
            mock_client.get.side_effect = httpx.RequestError("API unavailable")
            mock_client_class.return_value.__aenter__.return_value = mock_client

            result = await validator._check_external_apis_health()

            # All APIs should fail
            for api_result in result.values():
    ...
    def test_update_summary_with_failures(self, validator):
        """Test summary update with failures."""
        summary = {"passed": 0, "failed": 0, "warnings": 0}
    ...
    def test_update_summary_nested_results(self, validator):
        """Test summary update with nested results."""
        summary = {"passed": 0, "failed": 0, "warnings": 0}
